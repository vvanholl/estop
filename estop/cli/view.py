...
class ESTopTaskPopUp(urwid.WidgetWrap):
    def __init__(self, parent, task):
        ...
        items = self.create_widget_array(14,
                                              [
                                                  ...
                                                  ('Start time', time.strftime(
                                                      "%Y-%m-%d %H:%M:%S", self.task.start_time)),
                                                  ('Elapsed', "{0:.2f}ms".format(
                                                      self.task.running_time)),
                                                  ('Cancellable', {True: 'YES', False: 'NO'}[
        ...
        self.__super.__init__(urwid.LineBox(
            urwid.AttrWrap(self.frm_main, 'title')))

    def create_widget_array(self, title_width, titlevalues):
        t = []
...
class ESTopViewTaskWidget(urwid.TreeWidget):
        ...
        return urwid.Columns(
            [
                (42, urwid.Padding(
                    urwid.Columns(
                        [
                            (2, [self.unexpanded_icon, self.expanded_icon]
                             [self.expanded]),
                            self.popup_widget
                        ]
                    ),
    ...
    def unhandled_keys(self, size, key):
        if key == "enter":
            ...
            self.controller.poller.cancel_task(self.task.id)
        else:
            return key
    ...
    def __init__(self, controller, node):
        ...
        self._w._original_widget = urwid.Columns(
            [
                ...
                ('weight', 4, urwid.Text(
                    ('column_header', 'Action'), align='center')),
                ('weight', 3, urwid.Text(
                    ('column_header', 'Node'), align='center')),
...
class ESTopViewTaskNode(urwid.ParentNode):
    def __init__(self, controller, task, parent=None, depth=0):
        self.controller = controller
        self.depth = depth
        key = task.id
        urwid.ParentNode.__init__(
            self, task, key=key, parent=parent, depth=depth)
    ...
    def load_child_node(self, key):
        ...
        return ESTopViewTaskNode(self.controller, subtask, self, self.depth+1)

    def load_widget(self):
        return ESTopViewTaskWidget(self.controller, self)
...
class ESTopViewRootNode(urwid.ParentNode):
    def __init__(self, controller, task, parent=None, depth=0):
        ...
        self.depth = depth
        urwid.ParentNode.__init__(
            self, task, key='/', parent=parent, depth=depth)
    ...
    def load_child_node(self, key):
        task = self.get_value()
        subtask = [subtask for subtask in task.subtasks if subtask.id == key][0]
        return ESTopViewTaskNode(self.controller, subtask, self, 1)

    def load_widget(self):
        return ESTopViewRootWidget(self.controller, self)
...
class ESTopView():
    ...
    def __init__(self, controller):
        ...
        self.txt_cluster_name = urwid.Text(
            cluster_info['name'], align='center')
        ...
        self.map_cluster_status = urwid.AttrMap(
            self.txt_cluster_status, 'cluster_unknown')
    ...
    def refresh(self):
        ...
        self.map_cluster_status.set_attr_map(
            {None: 'cluster_' + cluster_health['status']})
        self.txt_cluster_nodes.set_text(
            "{0} nodes".format(cluster_health['number_of_nodes']))

        content = urwid.TreeWalker(ESTopViewRootNode(self.controller, tasktree))
