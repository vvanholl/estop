...
class ESTopPoller:
	...
	def get_cluster_data(self, path):
		...
		if r.status_code == 200:
			try:
				...
				raise Exception("Could not decode JSON data from endpoint={0} with path={1}".format(self.endpoint, path))
		else:
			raise Exception("Could not get data from endpoint={0} with path={1} (HTTP code={2})".format(self.endpoint, path, r.status_code))
	...
	def get_cluster_info(self):
		...
		try:
			...
			raise Exception("Could not fetch cluster info from endpoint={0}".format(self.endpoint))
	...
	def get_cluster_health(self):
		...
		try:
			...
			raise Exception("Could not fetch cluster health from endpoint={0}".format(self.endpoint))
	...
	def get_cluster_tasktree(self):
		...
		try:
			...
			raise Exception("Could not fetch cluster tasks from endpoint={0}".format(self.endpoint))
